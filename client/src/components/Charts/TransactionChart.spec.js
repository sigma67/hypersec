...
const ComponentNaked = unwrap(TransactionChart);
...
const setup = () => {
	...
	const wrapper = shallow(<TransactionChart {...props} />);
...
describe('TransactionChart', () => {
	test('TransactionChart component should render', () => {
		const { wrapper } = setup();
	...
	test('setInterval called', () => {
		const { props } = setup();
		const { getTransactionperMin } = props;
	...
	test('calls componentDidMount', () => {
		jest.spyOn(TransactionChart.prototype, 'componentDidMount');
		expect(TransactionChart.prototype.componentDidMount.mock.calls.length).toBe(
			0
		);
	});
	...
	test('syncData calls the selectors', () => {
		...
		const { getTransactionPerHour, getTransactionPerMin } = props;
