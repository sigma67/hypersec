...
const ComponentNaked = unwrap(BlockChart);
...
const setup = () => {
	const props = {
		...
		getInterblockTime: jest.fn(),
		currentChannel: 'mychannel'
	};

	const wrapper = shallow(<BlockChart {...props} />);
...
describe('BlockChart', () => {
	test('BlockChart component should render', () => {
		const { wrapper } = setup();
	...
	test('Nav tabs toggle to the corresponding state', () => {
		...
		wrapper
			.find('NavLink')
			.findWhere(n => n.contains('INTER-BLOCK TIME (MIN)'))
			.first()
			.simulate('click');
	...
	test('calls componentDidMount', () => {
		jest.spyOn(BlockChart.prototype, 'componentDidMount');
		expect(BlockChart.prototype.componentDidMount.mock.calls.length).toBe(0);
	});
	...
	test('syncData calls the selectors', () => {
		...
		const { getBlocksPerHour, getBlocksPerMin, getInterblockTime } = props;
		wrapper.instance().syncData('newData');
		expect(getBlocksPerHour).toHaveBeenCalled();
		expect(getBlocksPerMin).toHaveBeenCalled();
		expect(getInterblockTime).toHaveBeenCalled();
	});
});
