...
function TransactionsView({
  ...
  const binTimeFormat = date => {
    ...
    switch (msPerBin) {
      case 60000: //1m
        return timeFormat(
          `${startDate.format('DD.MM., kk:mm')}-${endDate.format('kk:mm')}`
        );
      case 3600000: //1h
        return timeFormat(
          `${startDate.format('DD.MM., kk:mm')}-${endDate.format('kk:mm')}`
      ...
      case 43200000: //12h
        return timeFormat(
          `${startDate.format('DD.MM., kk:mm')} - ${endDate.format(
            'DD.MM., kk:mm'
          )}`
        );
      case 86400000: //24h
  ...
  const binTrx = useCallback(() => {
    ...
    for (
      let i = Math.floor(start.valueOf() / msPerBin);
      i <= Math.floor(end.valueOf() / msPerBin);
      i++
    ) {
  ...
  useEffect(() => {
    ...
    fetchData();
    if (end.valueOf() - start.valueOf() > 10800000 && msPerBin < 3600000)
      setMsPerBin(3600000); //if msPerBin = 1min and range > 3h set msPerBin = 1h
    if (end.valueOf() - start.valueOf() > 604800000 && msPerBin < 43200000)
      setMsPerBin(43200000); //if msPerBin = 1h and range > 7d set msPerBin = 12h
  ...
  const handleBrushSelection = useCallback(
    selectedBinTimestamps => {
      ...
      metrics.forEach(bin => {
        ...
        if (
          binTimestamp >= selectedBinTimestamps[0] &&
          binTimestamp <=
